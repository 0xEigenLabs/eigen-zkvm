...
use plonky::witness::load_input_for_witness;
use plonky::Field;
use rayon::prelude::*;
...
pub fn join_zkin(
    ...
    let zkin1_map: HashMap<String, serde_json::Value> = serde_json::from_str(&inputs_str)?;

    let inputs_str = std::fs::read_to_string(zkin2).unwrap();
    let zkin2_map: HashMap<String, serde_json::Value> = serde_json::from_str(&inputs_str)?;
...
pub mod tests {
    ...
    fn test_stark_gen() {
        ...
        .unwrap();
        println!("starkproof");
        let a = stark_struct.serialize(()).unwrap();
        println!("{}", a);
