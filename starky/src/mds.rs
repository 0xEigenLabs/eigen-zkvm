...
pub(crate) fn quintic_s_box<F: PrimeField>(l: &mut F, pre_add: Option<&F>, post_add: Option<&F>) {
    ...
    let mut tmp = *l; // Create a copy of l
    tmp.square(); // Square tmp in-place (l^2)
    let mut tmp2 = tmp; // Create another copy
    tmp2.square(); // Square tmp2 in-place (l^4)
    l.mul_assign(&tmp2); // l^5
...
fn generate_mds<F: PrimeField>(t: usize) -> Matrix<F> {
    ...
    let xs: Vec<F> = (0..t as u64)
        .map(|x| F::from_str(&x.to_string()))
        .collect::<Option<Vec<F>>>() 
        .expect("Failed to parse string to F");
    let ys: Vec<F> = (t as u64..2 * t as u64)
        .map(|x| F::from_str(&x.to_string()))
        .collect::<Option<Vec<F>>>() 
        .expect("Failed to parse string to F");
...
mod tests {
    ...
    fn test_factor_to_sparse_matrices_aux(width: usize, n: usize) {
        ...
        for _ in 0..width {
            initial.push(Fr::rand(&mut rng));
        }

        let mut round_keys = Vec::with_capacity(width);
        for _ in 0..(n + 1) {
            round_keys.push(Fr::rand(&mut rng));
        }
