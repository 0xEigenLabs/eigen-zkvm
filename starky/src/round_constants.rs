...
pub fn generate_constants<F: PrimeField>(
    ...
    let mut slice = F::Repr::default();
    let byte_slice: &mut [u8] = unsafe {
        std::slice::from_raw_parts_mut(
            slice.as_mut().as_mut_ptr() as *mut u8,
            slice.as_ref().len() * std::mem::size_of::<u64>(),
        )
    };
    let n_bytes = byte_slice.len();
    if n_bytes != 32 {
        ...
        1 => {
            for _ in 0..num_constants {
                loop {
                    ...
                    let mut byte_slice: &mut [u8] = unsafe {
                        std::slice::from_raw_parts_mut(
                            repr.as_mut().as_mut_ptr() as *mut u8,
                            repr.as_ref().len() * std::mem::size_of::<u64>(),
                        )
                    };
                    grain.get_next_bytes(byte_slice);
                    byte_slice.as_mut().reverse();
                    let mut tmp = F::Repr::default();
                    let _ = tmp.read_le(&byte_slice[..]).map_err(|e| format!("could not read {}", &e));
                    // let u64_slice: &mut [u64] = unsafe {
                    //     std::slice::from_raw_parts_mut(
                    //         byte_slice.as_mut_ptr() as *mut u64,
                    //         byte_slice.len() / std::mem::size_of::<u64>(),
                    //     )
                    // };
                    // let mut u64_array: [u64; 4] = [0; 4];
                    // u64_array.copy_from_slice(u64_slice);

                    if let Ok(f) = F::from_repr(tmp) {
...
mod tests {
    ...
    fn test_round_constants() {
        ...
        let path = Path::new("/Users/ioio33/eigen-zkvm/starky/src/round_constants-1-1-255-9-8-57-73EDA753299D7D483339D80809A1D80553BDA402FFFE5BFEFFFFFFFF00000001.txt");
