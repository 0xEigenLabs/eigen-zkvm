...
fn prove_plonk_bn254(data_dir: &str, witness_path: &str) -> ProofBn254 {
    ProofBn254::Plonk(recursion_gnark_ffi::ffi::prove_plonk_bn254(data_dir, witness_path))
}

fn prove_groth16_bn254(data_dir: &str, witness_path: &str) -> ProofBn254 {
    ProofBn254::Groth16(recursion_gnark_ffi::ffi::prove_groth16_bn254(data_dir, witness_path))
}
...
fn run_verify(args: VerifyArgs) {
    ...
    let result = match (args.system.as_str(), proof) {
        ...
        ("groth16", ProofBn254::Groth16(proof)) => verify_groth16_bn254(
            &args.data_dir,
            &proof.raw_proof,
            &args.vkey_hash,
            &args.committed_values_digest,
