...
use powdr_number::{CsvRenderMode, GoldilocksField, KnownField};
...
pub fn zkvm_execute_and_prove(task: &str, _suite_json: String, output_path: &str) -> Result<()> {
    // log::debug!("Compiling Rust...");
    // let force_overwrite = true;
    // // let with_bootloader = true;
    // let options = CompilerOptions::new(KnownField::GoldilocksField, RuntimeLibs::new(), true);
    ...
    let executable = powdr_riscv::compile_rust_crate_to_riscv(
        &format!("program/{task}/Cargo.toml"),
        Path::new(output_path),
        None,
    );
    ...
    let _file_name = format!("{task}.asm");
    let mut pipeline = Pipeline::<GoldilocksField>::default()
        .with_output(output_path.into(), true)
        .with_backend(powdr_backend::BackendType::EStarkStarkyComposite, None)
        .with_witness_csv_settings(true, false, CsvRenderMode::Hex)
        .from_asm_string(asm, Some("{output_path}/{_file_name}".into()));
    pipeline.compute_witness().unwrap();
...
mod tests {
    ...
    #[test]
    fn test_zkvm_keccak_prove() {
        env_logger::try_init().unwrap_or_default();
        zkvm_execute_and_prove("keccak", "".to_string(), "/tmp/test_keccak").unwrap();
    }
