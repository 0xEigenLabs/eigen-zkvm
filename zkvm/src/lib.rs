use anyhow::Result;
use powdr::backend::{
    // composite::{ CompositeProof, CompositeVerificationKey},
    BackendType,
};
...
use powdr::riscv::{compile_rust, CompilerOptions};
use powdr::Pipeline;
...
fn generate_witness_and_prove_raw<F: FieldElement>(
    ...
    *pipeline = pipeline.clone().with_backend(
        BackendType::Plonky3,
        Some("stark_gl".to_string()),
    );
    pipeline.compute_proof()?;
...
pub fn zkvm_execute_and_prove<S: serde::Serialize + Send + Sync + 'static>(task: &str, suite_json: S, output_path: &str) -> Result<()> {
    ...
    //     force_overwrite,
    //     &CompilerOptions::base().with_poseidon(),
    //     false,
    //     with_bootloader,
    // )
    // .unwrap();
    ...
    let (asm_file_path, asm_contents) = compile_rust(
        &format!("program/{task}"),
        CompilerOptions::new_gl().with_poseidon().with_continuations(),
        Path::new(output_path),
        force_overwrite,
        None,
    )
    ...
    */
    ...
    rust_continuations(&mut pipeline, generate_witness_and_prove_raw, bootloader_inputs).unwrap();
...
mod tests {
    ...
    fn test_zkvm_ark_prove() {
        ...
        let mut file = fs::File::open("/mnt/nfs/sy/eigen-zkvm/zkvm/src/pvk.bin").unwrap();
        let mut data = vec![];
        file.read_to_end(&mut data).unwrap();
        zkvm_execute_and_prove("ark", data, "/tmp/test_ark").unwrap();
    }
